...
### REQ-0039  案件/スタッフ一覧の強化＋案件カレンダービュー
- 2026-06-11 / 会話（「案件レコードのリストビューにフィルターとグルーピングがない。スタッフも同じように。その他、カレンダービューを作って」）
- 内容：/assignments と /staff に他一覧と同じ ListViewToolbar（フィルター・グルーピング・ソート、SQL pushdown＋JS フォールバック）を導入（GroupedTable ベースの AssignmentsTableView / StaffTableView 新設）。/assignments にはリスト⇄**月間カレンダー**のビュー切替を追加（業務日 service_date ベース、状態色チップ、前月/翌月移動、フィルタ併用可）。カレンダーは汎用 `src/components/MonthCalendar.tsx` として実装（他ブックへの転用可）。
- 状態：実装・出荷済み。
