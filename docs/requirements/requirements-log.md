...
- **OPEN-A4**：進め方＝基盤レジストリ化(Phase1-2)を先か、手配機能を先か。
- **OPEN-B1**：リマインド既定タイミング（前日 P1D ＋当日 2h前 PT2H の両方を既定？）。
- **OPEN-B2**：リピート人材を `staff` として保存するか（推奨：保存）。
- **OPEN-B3**：請求書PDF/会計連携を MVP に含めるか（推奨：後回し、CSV で代替）。
- **OPEN-B4**：Gemini 鍵は env 固定か設定画面登録か（推奨：MVP は env 固定）。
- **OPEN-B5**：権限区分は既存ロール（admin/editor/viewer）流用でよいか。
- **OPEN-C1**：単価モデルは「案件固定単価」を主・時給を任意でよいか。
- **OPEN-C2**：メール送信は Resend でよいか。
- **OPEN-C3**：個人情報の LLM 送信ポリシー（送る/マスキング）。
