...
### REQ-0070  領収書取込時に経費の紐づけ先レコードを選べるように
- 2026-06-13 / 会話（「領収書インポートするときに、どのレコードに経費として紐付けるかを選べるようにしたい。レコードなしのケースもある」）
- 内容：クイックランチャーの AI 作成確認画面で、経費にも関連先ピッカー（取引先/人物/商談を検索して1件紐づけ・未選択なら単独作成）を表示する。AI が本文から関連先を自動セットした場合も「変更・外す」で解除可能（レコードなしに対応）。
- 状態：実装済み（feature/accounting-phase-a。#134 Phase A と同梱で出荷予定）。

### REQ-0071  経費の関連先候補に整備（板金）と案件（人材）を追加
- 2026-06-13 / 会話（「板金や案件にも付けれるようにしてください」← REQ-0070 の続き）
...
### REQ-0086  AI作成で関連レコードを一括作成（ディールグラフ）
- 2026-06-19 / 会話（「理想は一括でいろいろ作ること」「『新規リードとして株式会社◯◯を訪問。さっそく商談。商品■■を提案。金額200万。先方検討中。確度20%』を処理したらレコードがすべて設定された状態にしたい」）
- 内容：1つの自由文から、関連する複数レコード（取引先・連絡先・商談＋商品明細・活動）を**関係性ごと一括で**下書き→確認→作成する。
- 合意（AskUserQuestion 2026-06-19）：
  - (a) 対象範囲＝**フルグラフ**（取引先＋連絡先＋商談＋商品明細＋活動）。
  - (b) 起動＝**既存AI作成が自動判定**（複数レコードを含む文章なら一括確認画面、単一なら従来どおり。入口を増やさない）。
  - (c) 既存照合＝**既存があれば紐付けを既定**（REQ-0085 の「既存 or 新規」を踏襲。確認画面で新規作成にも切替可）。
- 例文のマッピング：取引先=株式会社◯◯／商談=■■提案（amount 200万・probability 20・stage proposal/negotiation・account_id 紐付け）／商品明細=■■（opportunity_products フリー入力・単価200万）／活動=訪問（meeting・取引先+商談に related）。
...
### REQ-0088  AI を「統一アシスタント（エージェント）」化
- 2026-06-23 / 会話（「AI機能はそれぞれ単発。これをAgentとして管理できるか」）＋AskUserQuestion（狙い=統一アシスタント／進め方=ADR→PoC両方）
- 内容：自然文で「検索・作成・要約・レポート」を頼める1つのアシスタントを、型付きツール＋上限付きループのエージェントとして実装。安全性は既存原則（draft-then-apply＝書込は下書き／ツール単位のRBAC／ステップ上限）を流用。
- 進め方：ADR-0032 を先に記録 → 最小PoC（provider非依存のJSONツールループ＋読み取りツール＋上限ループ＋最小チャットUI、読み取り専用で安全、`complete` 注入でキー無しでもユニットテスト）。
- 状態：**設計済→PoC実装中**（2026-06-23）。
- 関連：ADR-0032 / REQ-0022(AI作成)・REQ-0059/0060(AI検索)・REQ-0086(ディールグラフ)・#88(レポート)
