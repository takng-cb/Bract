...
- 鍵はサーバー側のみ（Vercel 環境変数 `GEMINI_API_KEY`、env 固定・ADR-0013）。個人情報を含む本文の LLM 送信は**許可（機能上必須）＋安全策**（ADR-0012）：必要な本文・文脈のみ送信、学習不使用・東京リージョン優先、外部保持最小。Vertex AI(東京) 切替は providers 抽象で吸収。
