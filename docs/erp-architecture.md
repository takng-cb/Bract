...
NEXT_PUBLIC_INDUSTRY='real-estate'   licenses.features.modules =
...
`licenses.features` を拡張:
```ts
export type LicenseFeatures = {
  ai_summary?: boolean
  extra_industries?: string[]   // 既存（温存）
  modules?: string[]            // 追加：有効モジュール ID 配列（ランタイム合成の真実）
}
```
...
└────────────────────────────────────────────────┘
                    ↓ 同梱された範囲内で
┌─ ランタイム（細かい） ──────────────────────────┐
│ licenses.features.modules で個別 ON/OFF          │  ← 再ビルド不要。/admin/modules でトグル
└─────────────────────────────────────────────────┘
```
...
| 改修 | `src/lib/license/types.ts` | `LicenseFeatures.modules?: string[]` 追加 |
| 改修 | `src/lib/license/index.ts` | モジュール判定を既存機構の上に薄く追加（既存 API 不変） |
...
## 9. 互換性・移行原則（ストラングラー方式）

- 既存3業種の本番を壊さないことを最優先。各 Phase は独立に動作確認可能。
- `licenses.features.modules` 未設定時は **現行 `activeIndustry` から有効モジュールを導出する互換シム**を噛ませ、本番無影響で基盤を導入。
