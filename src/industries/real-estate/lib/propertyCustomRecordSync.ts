/**
 * properties テーブルと book_records テーブルの同期ヘルパー（real-estate 業種専用）
 *
...
/** properties 行を book_records にミラー（INSERT または UPDATE）
...
export async function syncPropertyToCustomRecord(p: PropertyRow): Promise<void> {
  ...
  const existing = await db.select({ id: book_records.id })
    .from(book_records).where(eq(book_records.id, p.id))

  if (existing.length > 0) {
    await db.update(book_records)
      .set({ data, updated_at: new Date() })
      .where(eq(book_records.id, p.id))
  } else {
