...
export default async function NewProductPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-2xl">
      ...
      <h1 className="text-2xl font-bold text-zinc-900 mb-6">商品を追加</h1>
      ...
      <form action={action} className="bg-white border border-zinc-200 rounded-lg shadow-xs p-6 space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input name="cost_price" type="number" min="0" step="0.01" className={field} />
          </div>
          <div>
            <label className="block text-xs text-zinc-500 mb-1">発注しきい値</label>
            <input name="reorder_level" type="number" min="0" defaultValue={0} className={field} />
          </div>
          <div>
        ...
        <div className="flex items-center gap-2 pt-2">
          ...
          <Link href="/products" className="px-4 py-2 border border-zinc-300 text-zinc-600 text-sm rounded-md hover:bg-zinc-50">キャンセル</Link>
        </div>
      </form>
    </div>
  )
