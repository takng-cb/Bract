      ...
      <RecordHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <Link href={`/products/${id}`} className="px-4 py-2 border border-zinc-300 text-sm font-medium rounded-md hover:bg-zinc-50 transition-colors">
              キャンセル
            </Link>
            <button
              type="submit"
      ...
      <form id={FORM_ID} action={action}>
        <RecordColumns
          ...
          left={
            <CreateInfoCard
              ...
              title="商品情報"
              fields={[
                { label: 'SKU', name: 'sku', required: true, defaultValue: p.sku },
                { label: 'カテゴリ', name: 'category', defaultValue: p.category },
                { label: '単位（個/箱/kg 等）', name: 'unit', defaultValue: p.unit },
                { label: '売価', name: 'unit_price', kind: 'number', min: 0, defaultValue: p.unit_price },
                { label: '原価', name: 'cost_price', kind: 'number', min: 0, defaultValue: p.cost_price },
                { label: '発注しきい値', name: 'reorder_level', kind: 'number', min: 0, defaultValue: p.reorder_level ?? 0 },
              ]}
            >
              <div>
        ...
        >
          <CreateInfoCard
            title="基本情報"
            fields={[
              { label: '商品名', name: 'name', required: true, defaultValue: p.name, fullWidth: true },
              { label: '備考', name: 'description', kind: 'textarea', defaultValue: p.description, fullWidth: true },
            ]}
          />
        </RecordColumns>
        ...
        <div className="mt-6 flex justify-center gap-3 border-t border-zinc-200 pt-5">
          ...
          <Link href={`/products/${id}`} className="px-6 py-2 border border-zinc-300 text-sm font-medium rounded-md hover:bg-zinc-50 transition-colors">
            キャンセル
          </Link>
        </div>
      </form>
