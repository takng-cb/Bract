...
/**
 * 会話形式の AI 検索（REQ-0059）。QuickLauncher の検索ステップから使う。
 *
 * 左=会話、右=結果プレビュー（件数＋先頭8件）の2ペイン。発話のたびに
 * aiSearchTurn が「更新後の条件セット全体」を返し、previewAiSearch で結果を
 * 即時表示する。条件チップは × で個別に外せる（AI を介さずローカル更新）。
 * 適用は従来どおり一覧ページのフィルタ（?f=）に流す draft-then-apply。
 */
import { useEffect, useRef, useState } from 'react'
import { Loader2, Send, Eye, X, Sparkles } from 'lucide-react'
import {
  aiSearchTurn, previewAiSearch,
...
export default function AiSearchChat({ apiName, label, listHref, onNavigate }: Props) {
  ...
  const refreshPreview = async (conds: SearchCondition[]) => {
    if (conds.length === 0) { setPreview(null); return }
    try {
      setPreview(await previewAiSearch(apiName, conds))
    } catch {
      setPreview(null)
    }
  }
  ...
  const send = async () => {
    ...
    setError(null)
    try {
      const res = await aiSearchTurn(apiName, nextTurns, conditions)
      setTurns([...nextTurns, { role: 'assistant', text: res.reply }])
      setConditions(res.conditions)
      await refreshPreview(res.conditions)
    } catch (e) {
  ...
  const removeCondition = async (idx: number) => {
    ...
    setConditions(next)
    setTurns((prev) => [...prev, { role: 'assistant', text: `条件「${removed.label} ${OP_LABEL[removed.op] ?? removed.op} ${removed.value}」を外しました。` }])
  ...
  const apply = () => {
    ...
    onNavigate(qs ? `${listHref}?${qs}` : listHref)
  }

  const placeholder =
    apiName === 'tasks' ? '例: 今週期限の未完了のToDo' :
    apiName === 'opportunities' ? '例: 交渉中で100万以上の商談' :
    '例: 先月以降に登録した有効な取引先'
    ...
    <div className="flex flex-col md:grid md:grid-cols-2 md:h-[62vh] border border-zinc-200 rounded-xl overflow-hidden">
      ...
      <div className="flex flex-col h-72 md:h-auto border-b md:border-b-0 md:border-r border-zinc-200">
          ...
          {turns.length === 0 && (
            <div className="flex items-start gap-2">
              ...
              <div className="rounded-lg bg-white border border-zinc-200 px-3 py-2 text-sm text-zinc-700">
                どんな{label}を探しますか？会話で条件を足したり外したりできます。
                <span className="block text-xs text-zinc-400 mt-1">{placeholder} → 「そのうち〜だけ」「やっぱり〜は外して」</span>
              </div>
            </div>
          )}
          {turns.map((t, i) => (
        ...
        <div className="flex items-center gap-2 border-t border-zinc-200 bg-white p-2">
          <input
            ...
            placeholder={turns.length === 0 ? placeholder : '条件を追加・変更…（例: そのうち今月分だけ）'}
      ...
      <div className="flex flex-col h-80 md:h-auto">
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {preview ? (
            <>
              <p className="text-xs text-zinc-500 mb-2">該当 <b className="text-zinc-800">{preview.total}</b> 件{preview.total > preview.rows.length ? `（先頭 ${preview.rows.length} 件を表示）` : ''}</p>
        ...
        <div className="border-t border-zinc-100 p-3">
          ...
          >
            <Eye className="w-4 h-4" aria-hidden />この条件で一覧を開く
          </button>
        </div>
      </div>
    </div>
