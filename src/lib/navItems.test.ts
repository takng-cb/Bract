...
describe('customObjectsToNavItems（カスタムオブジェクト → NavItem 変換、業種別 URL）', () => {
  ...
  it('未知のカスタムオブジェクトはどの業種でも /books/<api>', () => {
    expect(customObjectsToNavItems([generic], 'base')[0].href).toBe('/books/reservations')
    expect(customObjectsToNavItems([generic], 'real-estate')[0].href).toBe('/books/reservations')
    expect(customObjectsToNavItems([generic], 'auto-body')[0].href).toBe('/books/reservations')
  })
