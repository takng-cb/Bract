...
describe('buildExtraNavItems（モジュール外ナビ項目の既定順構築）', () => {
  it('dashboard を除く静的ナビ → カスタム の順で並ぶ', () => {
    const custom: NavItem[] = [{ href: '/books/reservations', label: '予約', icon: '📅' }]
    const result = buildExtraNavItems(custom, 'base')
    const staticHrefs = ALL_NAV_ITEMS.filter((i) => i.href !== '/dashboard').map((i) => i.href)
    expect(result.map((i) => i.href)).toEqual([...staticHrefs, '/books/reservations'])
...
describe('customBooksToNavItems（カスタムブック → NavItem 変換、業種別 URL）', () => {
  ...
  it('real-estate モードで properties → /properties（業種専用 URL）', () => {
    const result = customBooksToNavItems([props], 'real-estate')
    expect(result[0]).toEqual({ href: '/properties', label: '物件', icon: '🏠' })
  })

  it('real-estate モードで vehicles → /books/vehicles（汎用 URL、業種専用ルートではない）', () => {
    const result = customBooksToNavItems([veh], 'real-estate')
    expect(result[0].href).toBe('/books/vehicles')
  })

  it('auto-body モードで vehicles → /vehicles（業種専用 URL）', () => {
    const result = customBooksToNavItems([veh], 'auto-body')
    expect(result[0].href).toBe('/vehicles')
  })

  it('auto-body モードで parts → /parts（業種専用 URL）', () => {
    const result = customBooksToNavItems([parts], 'auto-body')
    expect(result[0].href).toBe('/parts')
  })

  it('auto-body モードで properties → /books/properties（業種専用ルートではない）', () => {
    const result = customBooksToNavItems([props], 'auto-body')
    expect(result[0].href).toBe('/books/properties')
  })

  it('base モードでは全部 /books/<api> に向く', () => {
    const result = customBooksToNavItems([props, veh, parts], 'base')
  ...
  it('未知のカスタムブックはどの業種でも /books/<api>', () => {
    expect(customBooksToNavItems([generic], 'base')[0].href).toBe('/books/reservations')
    expect(customBooksToNavItems([generic], 'real-estate')[0].href).toBe('/books/reservations')
    expect(customBooksToNavItems([generic], 'auto-body')[0].href).toBe('/books/reservations')
  })

  it('label_plural と icon はそのまま保持', () => {
    const result = customBooksToNavItems([{ api_name: 'foo', label_plural: 'バー', icon: '🎯' }], 'base')
    expect(result[0]).toEqual({ href: '/books/foo', label: 'バー', icon: '🎯' })
  })
})
