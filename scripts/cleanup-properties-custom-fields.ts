 ...
 * schema 列の複製 field_definitions と、それに連動する custom_field_values を削除する。
 *
 * 背景:
 *   migrate-properties-to-custom.mjs は properties テーブルの各列を
 *   field_definitions に複製登録し、過去の物件 CRUD でその custom_field_values
 ...
 *   - object_definitions の 'properties' 行も削除しない（サイドバー・リレーション参照は維持）
 *   - 削除対象は field_definitions (object_id = properties) と
...
async function main() {
  ...
  const objs = await sql`SELECT id FROM object_definitions WHERE api_name = 'properties' LIMIT 1`
  if (objs.length === 0) {
    console.log('object_definitions に properties が無いため、削除対象なし。終了。')
    return
  }
  const propertiesObjectId = objs[0].id as string
  console.log(`📍 properties object_id = ${propertiesObjectId}`)

  // 対象 field_definitions を確認
  ...
  `
  console.log(`\n📋 削除対象 field_definitions: ${fields.length} 件`)
  for (const f of fields) {
    console.log(`   - ${f.api_name} (${f.label})`)
  }

  if (fields.length === 0) {
    console.log('\n✅ field_definitions が無いため削除対象なし。終了。')
    return
  }
  ...
  console.log('🗑️  field_definitions を削除中...')
  await sql`DELETE FROM field_definitions WHERE object_id = ${propertiesObjectId}`
